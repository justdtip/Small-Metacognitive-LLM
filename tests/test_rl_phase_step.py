...
def test_rl_phase_step_reduces_mu():
    model = TinyLM(vocab_size=128, hidden=32)
    batch = _build_tmp_batch()
    pol0, stats0 = rl_phase_step(model, batch)
    pol1, stats1 = rl_phase_step(model, batch, policy=pol0)
